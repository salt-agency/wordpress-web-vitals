...
?>
      <form action="options.php" method="post">
        <h2><?= esc_html__('Web Vitals', 'sa-webvitals') ?></h2>
        <?php
    ...
    {
      register_setting('sa_webvitals', SA_WebVitals::OPTION_SETTINGS, [
    ...
    ?>
      <input type="hidden" name="<?= SA_WebVitals::OPTION_SETTINGS ?>[enabled]" value="0">
      <input type="checkbox" name="<?= SA_WebVitals::OPTION_SETTINGS ?>[enabled]" <?php if($enabled_setting):?>checked="1"<?php endif; ?>>
    <?php
    }
    ...
    ?>
      <select name="<?= SA_WebVitals::OPTION_SETTINGS ?>[load]">
        <?php foreach ($load_options as $option => $label) : ?>
          <option value="<?= esc_attr($option) ?>" <?php if ($load_setting === $option) : ?>selected<?php endif; ?>><?= esc_html($label) ?></option>
        <?php endforeach ?>
      </select>
    <?php
    }
    ...
    ?>
      <select name="<?= SA_WebVitals::OPTION_SETTINGS ?>[integration]">
        <?php foreach ($integration_options as $option => $label) : ?>
          <option value="<?= esc_attr($option) ?>" <?php if ($integration_setting === $option) : ?>selected<?php endif; ?>><?= esc_html($label) ?></option>
        <?php endforeach ?>
      </select>
<?php
    }
